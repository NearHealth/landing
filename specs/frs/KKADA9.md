...
- AC-KKADA9.2: A `ContactForm` container composes the atoms and owns all state (fields,
  ...
  and a `submitLabel` prop (default "Send message").
...
- AC-KKADA9.5: Every field has a visible label above it (not placeholder-only). First name,
  Last name, Email, Organization type, Partnership type are required (asterisk); Company and
...
- AC-KKADA9.8: On submit the button shows loading and is disabled in-flight (double-submit
  protection); resolve → success-message swap; reject → system-error shown, input preserved,
  button re-enabled.
...
- AC-KKADA9.12: `ContactPage` renders `<ContactForm/>` with its intent-driven hero (page reads
  ...
  `setTimeout` simulation is replaced by the `onSubmit` prop.
...
- Deliberate checklist breaks (design wins): 2-column layout (vs §1 single-column); asterisk on
  required (vs §1 "mark optional"). Organization/Partnership also get a required asterisk →
  minor divergence from Figma (which shows no asterisk on the dropdowns/Company).
- Checklist honored over design: visible labels above fields (vs the design's placeholder-only)
  → the rendered form shows labels the Figma frames don't have.
